...
def load_local_vlm(model_path: str | None = None):
    """Load local VLM with the same robust order as the capability notebook.

    Difference from the previous processorfix file:
    - no Qwen-specific model classes are hard-required;
    - no temporary processor JSON patching is required;
    - if AutoProcessor fails, fallback is AutoTokenizer + AutoImageProcessor;
    - model loading tries AutoModelForVision2Seq first, then AutoModelForCausalLM.
    """
    import torch
    ...
    try:
        ...
        try:
            model = AutoModelForVision2Seq.from_pretrained(str(model_path_obj), **common_kwargs)
    ...
    if model is None:
        model = AutoModelForCausalLM.from_pretrained(str(model_path_obj), **common_kwargs)
