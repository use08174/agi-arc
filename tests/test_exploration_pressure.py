...
class ExplorationPressureTest(unittest.TestCase):
    ...
    def test_agent_enters_counterfactual_mode_when_one_action_dominates(self) -> None:
        agent = GraphSearchAgent(config=AgentConfig())
        agent.steps_since_semantic_progress = agent.config.budget.semantic_patience_steps
        agent.recent_actions.extend(["ACTION1"] * 8)
        agent.recent_action_keys.extend(["ACTION1"] * 8)
        agent.recent_action_families.extend(["movement"] * 8)
